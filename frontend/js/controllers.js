...
angular
  ...
  .controller("headerctrl", function(
    ...
    if (screen.width > 600) {
      ...
      $rootScope.mobileUrl =
        splitUrl[0] + "mobile/index.html#/app/filterResult";
    }
  ...
  .controller("ModelViewCtrl", function(
    $scope,
    $window,
    hotkeys,
    TemplateService,
    ...
    $scope.forceClose = function(data) {
      ...
      NavigationService.calcGlobalAssignmentStatus(formData, function(
        timelineStatus
      ) {
        if (data.value == true) {
        ...
        if (value) {
          NavigationService.deleteModel($scope.ModelApi, id, function(data) {
            if (data.value) {
              ...
              toastr.success(
                $scope.modelCap + " deleted successfully.",
                $scope.modelCap + " deleted"
              );
            } else {
              toastr.error(
                "There was an error while deleting " + $scope.modelCap,
                $scope.modelCap + " deleting error"
              );
            }
          });
        }
    ...
    $scope.generateCustomersExcel = function() {
      NavigationService.saveJsonStore($scope.filter2, function(data) {
        window.open(
          adminurl +
            "Customer/generateCustomersExcel?id=" +
            data +
            "&accessToken=" +
            $.jStorage.get("accessToken"),
          "_blank"
        );
      });
    };
