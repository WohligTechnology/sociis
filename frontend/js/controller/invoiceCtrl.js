firstapp.controller("InvoiceViewCtrl", function(
  $scope,
  $window,
  hotkeys,
  TemplateService,
...
firstapp.controller("CreateInvoiceCtrl", function(
  $scope,
  $window,
  TemplateService,
  NavigationService,
  ...
  $scope.getAll = function(invoice, $index) {
    ...
    if ($scope.formData.invoiceList[$index].quantity) {
      $scope.calAmt(
        $scope.formData.invoiceList[$index].rate,
        $scope.formData.invoiceList[$index].quantity,
        $index
      );
    } else {
  ...
  $scope.calculateCreditPending = function(data) {
    ...
    NavigationService.getOneModel("Customer", data._id, function(data) {
      if (data.value == true) {
        ...
        $scope.creditPending =
          billedToCreditDetails.creditLimitAlloted -
          billedToCreditDetails.creditLimitExhausted;
      }
    });
  };
...
firstapp.controller("EditInvoiceExpenditureCtrl", function(
  $scope,
  hotkeys,
  $window,
  TemplateService,
        ...
        if (data.value === true) {
          ...
          toastr.success(
            "InvoiceExpenditure " +
              $scope.formData.name +
              " edited successfully.",
            "InvoiceExpenditure Edited"
          );
        } else {
          toastr.error(
            "InvoiceExpenditure edition failed.",
            "InvoiceExpenditure editing error"
          );
        }
      });
    }
