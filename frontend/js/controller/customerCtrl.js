firstapp
  ...
  .controller("CreateCustomerCtrl", function(
    $scope,
    hotkeys,
    $window,
    TemplateService,
    ...
    $scope.saveModel = function(formData) {
      NavigationService.modelSave("Customer", $scope.formData, function(data) {
        if (data.value === true) {
          ...
          toastr.error(
            "Customer" + data.error.message + " creation failed.",
  ...
  .controller("EditCustomerCtrl", function(
    $scope,
    hotkeys,
    $window,
    TemplateService,
    ...
    $scope.changePending = function() {
      $scope.formData.creditPending =
        $scope.formData.creditAlloted - $scope.formData.creditExhausted;
    };
    ...
    NavigationService.getCustomerProductDetailsAccordingToInvoices(
      customerObject,
      function(data) {
        $scope.invoiceProductDetails = data.data.results;
        $scope.totalItems = data.data.total;
        console.log(
          "$scope.invoiceProductDetails",
          $scope.invoiceProductDetails
        );
      }
    );
    ...
    $scope.saveModel = function(formValid) {
      NavigationService.modelSave("Customer", $scope.formData, function(data) {
        if (data.value === true) {
          $window.history.back();
          toastr.success(
            "Customer" + $scope.formData.name + " edited successfully.",
            "Customer" + " Edited"
          );
        } else {
          toastr.error(
            "Customer" + data.error.message + " edition failed.",
            "Customer" + " editing error"
          );
        }
      });
    };
